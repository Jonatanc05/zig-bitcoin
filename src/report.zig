...
pub fn print(privkey: u256) !void {
    ...
    try stdout.print("\n------------- FiniteFields -------------\n", .{});
    {
        ...
        const a = fe(10);
        const b = fe(5);
        try stdout.print("Element a: {}\n", .{a});
        try stdout.print("Element b: {}\n", .{b});
        try stdout.print("a + b: {}\n", .{a.add(b)});
        try stdout.print("a - b: {}\n", .{a.sub(b)});
        try stdout.print("b - a: {}\n", .{b.sub(a)});
        try stdout.print("a * b: {}\n", .{a.mul(b)});
        try stdout.print("a ** 2: {}\n", .{a.pow(2)});
        try stdout.print("a ** 3: {}\n", .{a.pow(3)});
        try stdout.print("a / b: {}\n", .{a.div(b)});
        try stdout.print("\n", .{});
    }

    try stdout.print("------------- EllipticCurves -------------\n", .{});
    {
        ...
        try stdout.print("Point p1: {}\n", .{p1});
        try stdout.print("Point p2: {}\n", .{p2});
        try stdout.print("p1 + p2: {}\n", .{p1.add(p2)});
        const p3 = CurvePoint.init(fe(47), fe(71), a, b);
        try stdout.print("Point p3: {}\n", .{p3});
        try stdout.print("p3 + p3: {}\n", .{p3.add(p3)});
        try stdout.print("2 p3: {}\n", .{p3.muli(2)});
        try stdout.print("3 p3: {}\n", .{p3.muli(3)});
        try stdout.print("18 p3: {}\n", .{p3.muli(18)});
        try stdout.print("19 p3: {}\n", .{p3.muli(19)});
        try stdout.print("20 p3: {}\n", .{p3.muli(20)});
        try stdout.print("21 p3: {}\n", .{p3.muli(21)});
        var G = CurvePoint.init(fe(15), fe(86), a, b);
        G.computeOrder();
        try stdout.print("order of the group generated by {}: {}\n", .{ G, G.order.? });
        try stdout.print("\n", .{});
    }

    try stdout.print("-------------- Cryptography --------------\n", .{});
    {
        const message = "The quick brown fox jumps over the lazy dog";
        try stdout.print("message: \"{s}\"\n", .{message});
        const z = CryptLib.hashAsU256(message);
        try stdout.print("h(message): {x}\n", .{z});
        const keys = CryptLib.generateKeyPair();
        try stdout.print("Public key: {}\nPrivate key: {x}\n", .{ keys.pubk, keys.prvk });
        try stdout.print("Signing the message...\n", .{});
        const sig = CryptLib.sign(z, keys.prvk);
        try stdout.print("Verifying the signature...\n", .{});
    ...
    try stdout.print("-------------- Serialization --------------\n", .{});
    {
        ...
        try stdout.print("p1.x: 0x{x:0>64}\n", .{p1.x.?.value});
        try stdout.print("p1.y: 0x{x:0>64}\n", .{p1.y.?.value});

        var p1_uncompressed: [65]u8 = undefined;
        p1.serialize(false, &p1_uncompressed);
        try stdout.print("serialized(p1): 0x", .{});
    ...
    try stdout.print("\n------------------- Transactions -------------------\n", .{});
    {
        ...
        try stdout.print("\n-----> Script\n", .{});
        {
            ...
            try stdout.print("script_pub_key: OP_SHA256, <Sha256(answer)>, OP_EQUAL, OP_VERIFY\n", .{});
            try stdout.print("script_pub_key in hex: ", .{});
            for (script_pub_key) |b| try stdout.print("{x:0>2}", .{b});
            try stdout.print("\n", .{});
            const script_sig = [_]u8{Op.OP_PUSHDATA1} ++ [1]u8{answer.len} ++ answer.*;
            try stdout.print("script_sig: OP_PUSHDATA1, <answer>\n", .{});
            try stdout.print("script_sig in hex: ", .{});
            for (script_sig) |b| try stdout.print("{x:0>2}", .{b});
            try stdout.print("\n", .{});
            const valid = Bitcoin.Script.validate(&script_sig, &script_pub_key, null, null, allocator);
            try stdout.print("valid: {any}\n", .{valid});
        }
        ...
        try stdout.print("\n-----> Signing a transaction\n", .{});
        {
            ...
            try stdout.print("entire transaction: ", .{});
            for (serialized) |b| try stdout.print("{x:0>2}", .{b});
            try stdout.print("\n", .{});
            ...
            try stdout.print("OP_CHECKSIG result: {s}\n", .{if (checksig) "valid" else "invalid"});
        }
    }
}
